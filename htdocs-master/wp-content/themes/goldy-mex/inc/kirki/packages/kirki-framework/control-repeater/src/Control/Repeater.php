...
class Repeater extends Base {
	...
	public function __construct( $manager, $id, $args = [] ) {
		...
		$this->row_label = [
			'type'  => 'text',
			'value' => esc_attr__( 'row', 'goldy-mex' ),
			'field' => false,
		];
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_html__( 'Add new %s', 'goldy-mex' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							[
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_html__( 'Select a Page', 'goldy-mex' ),
								'option_none_value' => '0',
								'selected'          => '',
							]
						);
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'goldy-mex' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
					<# _.each( data, function( field, i ) { #>
						...
						<div class="repeater-field repeater-field-{{{ field.type }}} repeater-field-{{ field.id }}">
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'goldy-mex' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_html_e( 'No Image Selected', 'goldy-mex' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'goldy-mex' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'goldy-mex' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'goldy-mex' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change Image', 'goldy-mex' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add Image', 'goldy-mex' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'goldy-mex' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_html_e( 'No File Selected', 'goldy-mex' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'goldy-mex' ); ?></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'goldy-mex' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'goldy-mex' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change File', 'goldy-mex' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add File', 'goldy-mex' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'goldy-mex' ); ?></button>
				</div>
			</li>
		</script>
